...
bpPlayer.muteRecording(mute:Boolean):Void
	...
	* The mute parameter specifiese if the stream that is being recorded should be muted or unmuted. Unmuting restores the stream volume to its previous value.
...
bpPlayer.startTalking(roleId:String, duration:Number):Void
	...
	* The duration parameter tells the video player the time window availablee to speak for the specified role.
...
**Video player states**
 * 0: PLAY_STATE. The video player is playing only a video
 * 1: PLAY_BOTH_STATE. The video player is playing two videos side-by-side using loose synchronization.
 * 2: RECORD_MIC_STATE. A response is being recorded, and the user is using only a microphone device.
 * 3: RECORD_BOTH_STATE. A response is being recorded, and the user is using a webcam device.
